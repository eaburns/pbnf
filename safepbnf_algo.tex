...
\begin{function}
  ...
  \KwResult{Whether or not the given nblock is free}
...
\begin{function}
  ...
  \KwIn{\block -- an nblock}
  \If{{\bf not} $\hot(\block)$ {\bf and} $\sigma(\block) > 0$}{
...
\begin{function}
  ...
  \KwData{\minexpansions -- the minimum number of expansions to
    perform per-nblock}
  \KwResult{Whether or not the nblock should be switched}
...
\begin{function}
  ...
  \KwIn{\block -- an nblock}
  \ForEach{$b' \in \interferenceScope(\block)$}{
    $\sigma_h(b') \leftarrow \sigma_h(b') - 1$\;
  }
\end{function}
...
\begin{function}
  ...
  \If{$\block \neq$ NULL}{
    ...
    \release(\block)\;
    \lIf{$\bestScope < \best(\FreeList)$}{
      $\sethot(\bestScope)$\;
    }
    \lElse {
      $\bestScope \leftarrow$ NULL\;
    }
  }
