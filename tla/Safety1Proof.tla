                  ...
                  <4>1. CASE isHot[x]
                        ...
                        <5>2. CASE x \notin Free(Acquired \ acquired[i])
                              <6>1. Cardinality(Overlap(x, Acquired)) > 0
                                    <7>1. Busy(Acquired \ acquired[i]) \subset Busy(Acquired \ acquired[i])
                                          PROOF OBVIOUS
                                          \* There are less acquired blocks, and therefore there are less busy blocks.
                                    <7>2. Overlap(x, Acquired \ acquired[i]) \subseteq Overlap(x, Acquired)
                                          <8>1. CASE acquired[i] \in Succs[x]
                                    ...
                                    <7>3. HotInterference(Aqcuired \ acquired[i]) \subseteq HotInterference(Acquired)
                                          PROOF BY <7>1 and <7>2 \* and the definition of HotInteference(A).
                                    <7>4. x \notin HotInterference(Acquired)
                                    ...
                                    <7>5. x \notin HotInterference(Acquired \ acquired[i])
                                          PROOF BY <7>4 and <7>3
                                    <7>6. QED BY <5>2 and <7>5 \* ((<5>2 => <6>1 \/ ~<7>1) /\ <5>2 /\ <6>1) => <6>1
                              <6>2. Hot(Acquired')' \subseteq Hot(Acquired)
                                    PROOF OBVIOUS \* Since isHot' is the same as or more false than isHot.
                                    \* Is there a better way to explain this?!
                              <6>3. HotInterference(Acquired')' \subseteq HotInterference(Acquired)
                                    PROOF BY <6>2 \* and the definition of HotInterference(A).
                                    \* There are less hot blocks, so naturally there are less blocks which interfere with hot blocks.
                              <6>4. x \notin HotInterference(Acquired')'
                                    PROOF BY <6>2 and <6>3
                              <6>5. QED BY <6>1 and <6>4 \* The RHS is true and the implication holds trivially.
