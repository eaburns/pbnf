...
def find_invariants(task):
  ...
  print len(candidates), "Initial invariant candidates found"
  restricted = False
  ...
  while candidates:
    if len(candidates) > 10000:
      print "Too many invariant candidates, aborting invariant generation"
      return
    candidate = candidates.pop()
    if restricted and len(candidate.parts) > 10:
      #print "Not considering invariant candidate of size", len(candidate.parts)
      continue
