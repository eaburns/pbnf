...
Parallelization of algorithms can be achieved by many different means. The methods used for thes algorithms we will test are the following: shared access to all data, global synchronization by one central thread, and paritioning of the state space. Sharing access to all data structures is the simplest method, but requires mutual exclusion locks on each shared object. When multiple threads attempt to obtain the same lock, this causes contention in which threads must wait for the object to be used and the lock to be released before they can proceed with their work. Synchronization implies that a main thread accesses the data and gives it equally to each worker thread to perform work on. This entails a lot of time spent waiting, both the main thread waiting on workers to finish and workers waiting on the main thread to give them more work. By partitioning the state space, one allows data to remain in separate data structures to which threads can be given mostly or entirely exclusive access throughout some or all of the algorithm's execution time. This requires more advance knowledge and may still require some means of sharing or passing data. The algorithms we examine will all utilize one or more of these methods for parallelizing search.
\section{Methods}
...
All algorithms were tested on two identical machines with dual quad core processors, allowing for a maximum concurrent execution of 8 threads. Tests were performed up to 10 threads, with decreases in performance being expected after 8.
While the goal was to test algorithms that return optimal solutions, some algorithms (PA*, PRA*, and KBFS) were so slow in finding a suboptimal solution that their performance was only recorded for the first solution found. Performance can be considered a lower bound on the cost of finding optimal solutions, and in all cases the first solution will be potentially less optimal as the number of threads increases.
...
One adaptation to PSDD \cite{zhou:sdd} suggested by Zhou and Hansen was to search layers in a best-first order (BFPSDD) by defining layers based on $f$, rather than $g$. The search still expands all nodes in a layer before moving to the next, but the layers themselves consist of different nodes than in the breadth first search of PSDD. Essentially, less work may be performed by exploring only the most promising nodes, but optimality can be maintained so long as the heuristic is optimal. This modification reduces some of the search cost, but does not resolve the issues with layers in a non-unit cost setting. Another modification was proposed to allow for grouping ranges of f-values into a single layer, but that was not included in our implementation. Our implementation of BFPSDD does, however, include the bounding methods used in DynPSDD.
...
There is a clearly discernible best ratio of nblocks to threads in life cost grid world problems tested, but the algorithm's behavior overall is less desirable. Figure \ref{fig:BFPSDD-nblock-life} shows that the ratio of 2 nblocks per thread is best for BFPSDD in the life cost instances tested. One nblock per thread caused such bad performance that it could not be fully tested.
...
PBNF does much better than A* in both unit and life cost grid world, with a continued decrease in time spent on unit cost problems and an upward slope after five threads on life cost problems. This is most likely because maintaining a ratio of nblocks to threads means that the abstraction may be too large for higher numbers of threads. Because the costs of nodes will vary more, good nodes are likely to be fewer and more distributed among the nblocks, especially with the abstract states being defined by row, which also defines action cost.
...
When run on sliding tiles puzzles, PBNF does not always outperform A*, with a sharp increase in times for more than 3 threads. We believe that this is due to either problems with the size of abstraction used or the inaccuracy of the Manhattan Distance heuristic for sliding tiles puzzles.
...
Parameters in PBNF have very similar results to those found for PBNF, with one exception. SafePBNF often performs best when more expansions are required before switching than would be used for PBNF, because the algorithm is designed to switch nblocks in even more situations than PBNF.
...
Performance for SafePBNF in grid worlds follows the same trends as PBNF, beating A* at all points and with consistent improvement in unit cost and less scaability in life cost. Performance in sliding tiles likewise follows the same basic trend, except that the first 3 threads do not seem to positively or negatively impact performance. We do not know why this happens, but variance for results on those values is much higher than others.
\section{Comparison}
Having determined the best parameters to use with each algorithm for the domains we tested them on and examining their individual behavior, the next step is to compare their performance across those domains. We show results for unit cost grid world, life cost grid world, and the 7x2 sliding tile puzzle for all algorithms which performed well enough to complete execution on the given domain. All parameters were set to the best known value to compare best case performance. We test with 1 to 8 threads because the machine we tested on has 8 cores.
...
\section{Conclusion}
From the example of KBFS, we can see that having one main thread which synchronizes the work of other threads can be very bad for performance. The amount of parallel work that must be done to outweigh this cost is so large we were not able to measure it with 8 threads. While this scheme avoids some contention, it does so at great up front expense.
Our results from varying the size of the abstraction used show that it is, indeed, an important factor in PSDD and PBNF algorithms. If the abstraction is too small, the search is very slow for small numbers of threads, but can speed up as more threads are added. On the other hand, if the ratio of nblocks to threads is too large, these algorithms will perform poorly for higher numbers of threads. This indicates that smaller abstractions should be used if scalability is important to the user.
In our comparisons, we found that BFPSDD is competitive in unit cost domains. It performs better than all other algorithms on the sliding tile puzzle and is only marginally beaten by PBNF and SafePBNF in unit cost grid world. However, PBNF and SafePBNF were the only algorithms able to beat serial A* in life cost grids, because BFPSDD is unable to handle the many layers created by variable action costs.
Finally, we discovered that it is quite difficult to determine the parameters to use for all PSDD and PBNF variants. They do not seem to translate well across domains and would be non-trivial to determine before execution, although some general guidelines can be inferred for abstraction size.
\section{Future Work}
...
Because adjusting the size of the abstraction for PSDD and PBNF variants more precisely was not possible for sliding tiles, only one size of abstraction was tested. However, the size of the abstraction used will certainly have an effect in this domain, too. We cannot be sure whether the impact will be as great or what size is good, though, without further inquiry.
While attention was paid to the size of the abstraction used, examination of the properties that make an abstraction useful and how big of an effect these properties have on execution time could be very informative. In addition, PRA* could be tested with hash functions of varying quality. Both abstractions and hashes could be tested to see how well they split up the state space and how evenly they distribute good nodes.
